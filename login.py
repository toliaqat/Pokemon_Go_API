...
def get_login_session(username, password):
	access_token, ltype = get_acces_token(username, password)
	if access_token is not None:
		prot1 = logic.gen_first_data(access_token, ltype)
		login_session = api.get_rpc_server(access_token, prot1)
		return login_session
	raise Excption("unable to obtain access token")
...
def get_acces_token(usr, pws):
	...
	if '@' in usr:
		...
		if platform.system() == 'Windows':
			google_data = login_google(usr,pws)
			if google_data is not None:
				access_token = google_data['id_token']
		else:
			access_token = login_google_v2(usr,pws)
		if access_token is not None:
			ltype = 'google'
	else:
		print '[!] I am a poketrainer..'
		access_token = login_pokemon(usr,pws)
		ltype = 'ptc'
